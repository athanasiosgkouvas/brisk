...
- **Your `@brisk` username & photo.** Pick a handle once at sign‑up (`you@brisk`) — and, optionally, a
  profile photo. Friends send you money **by name**, and it's your name and picture (never a `0x…`
  address) that show whenever you pay, send, or receive.
...
- **A clean history.** Your activity feed shows who you paid — **businesses** by their real name and logo,
  **friends** by their `@brisk` username and photo — plus an on‑chain receipt for every purchase. Tap **See all** for
