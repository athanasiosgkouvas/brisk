...
/// Record a payment that already settled out-of-band (e.g. a native-gasless
/// `send_funds` transfer), minting the soulbound `Receipt` + emitting
/// `PaymentMade` WITHOUT moving any coin. This is the best-effort "receipt leg"
/// of a two-leg payment: the money moves in a separate feeless transfer (the
/// source of truth), and this records the merchant-bound receipt so the payment
/// shows up as verifiable commerce. Because it touches no `Balance`/`Coin`, the
/// tx carries no `FundsWithdrawal` and is trivially sponsorable.
///
/// `payee`/`merchant` are still read from the on-chain `Merchant` profile (not
/// caller-supplied); `amount`/`memo`/`invoice_id` are attested by the payer.
#[allow(lint(self_transfer))]
